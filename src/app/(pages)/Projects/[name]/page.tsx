...
export default function Page({ params }: { params: { name: string } }) {
    ...
    <div className="w-full h-full grow flex flex-row items-center justify-center px-12" >
      <div className={`flex-[3_3_0%] flex items-center justify-center`}>
        <Image className="rounded-lg mx-8" src={`${process.env.NODE_ENV === 'production' ? '/wynnie_next' : ''}/images/${params.name}_1.avif`} width={300} height={600} alt={`${params.name}_1.avif`} unoptimized/>
        <Image className="rounded-lg mx-8" src={`${process.env.NODE_ENV === 'production' ? '/wynnie_next' : ''}/images/${params.name}_2.avif`} width={300} height={600} alt={`${params.name}_2.avif`} unoptimized/>
      </div>
      <div className={`flex-[2_2_0%] h-3/5 bg-E0E1DD rounded-2xl shadow-md flex items-start justify-start flex-col`}>
