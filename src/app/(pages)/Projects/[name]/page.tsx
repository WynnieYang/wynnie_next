...
export default function Page({ params }: { params: { name: string } }) {
    ...
    <div className="w-full h-full grow flex flex-row items-center justify-center px-12" >
      <div className={`flex-[3_3_0%] flex items-center justify-center`}>
        <Image className="rounded-lg mx-8" src={`/images/${params.name}_1.png`} width={300} height={600} alt={`${params.name}_1.png`}/>
        <Image className="rounded-lg mx-8" src={`/images/${params.name}_2.png`} width={300} height={600} alt={`${params.name}_2.png`}/>
      </div>
      <div className={`flex-[2_2_0%] h-3/5 bg-FBFEFB rounded-2xl shadow-md flex items-start justify-start flex-col`}>
        ...
        <div className="border-t w-full border-slate-400 grow flex items-start justify-between flex-col">
          <div></div>
          <div className="w-full flex items-end justify-end"><Image src={`/images/${params.name}ing_1.svg`} width={60} height={80} alt={`${params.name}ing_1.svg`}/></div>
        </div>
      </div>
    </div>
